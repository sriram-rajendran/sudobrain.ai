...
| Sources | recordings, Slack, Gmail, Fathom, Linear, Calendar, local repos, connector SDK runtime, plugin registry | attachment extraction, document library, bookmarks, OCR text handoff, folder watch config, and project-context scoring | dynamic external plugin execution |
...
| Trust | source audit, privacy docs, public repo verifier, knowledge export, trust report, reversible review action log, approval bundles, workflow approval outbox | citation cards and provenance details | advanced provenance graph UI |
| Tryability | bootstrap script, demo seed data, smoke test, release readiness audit | full Docker Compose backend | signed release artifacts |
